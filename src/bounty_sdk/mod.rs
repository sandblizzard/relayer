...
impl BountySdk {
    ...
    pub fn complete_bounty(
        ...
        // validate input
        let solver_1 = match solvers.get(0) {
            Some(solver) => solver,
            None => return Err(SBError::NeedAtLeastOneSolver("complete_bounty".to_string())),
        };
        ...
        let compelete_bounty_accounts = bounty::accounts::CompleteBounty {
            ...
            escrow: escrow_pda.0,
            solver1: *solver_1,
            solver2: solvers.get(1).copied(),
            solver3: solvers.get(2).copied(),
            solver4: solvers.get(3).copied(),
